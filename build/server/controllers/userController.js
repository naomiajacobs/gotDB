...
module.exports = {
  retrieve: function retrieve(req, res) {
    ...
    User.findById(id, { attributes: { exclude: ['password'] } }).then(function (user) {
      if (user) {
  ...
  update: function update(req, res) {
    ...
    User.update(updates, { where: { id: id } }).then(function (updateArray) {
  ...
  delete: function _delete(req, res) {
    var id = req.params.userId;
    User.destroy({ where: { id: id } }).then(function (numberDestroyed) {
      if (numberDestroyed) {
        res.status(201).json({
          success: true,
          userId: id
