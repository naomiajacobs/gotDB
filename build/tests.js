...
describe('Auth DB Queries', function () {
  ...
  it('Should insert a new user into the DB', function (done) {
    ...
    authDB.addNewUser(users[0]).then(function (results) {

      authDB.findUser({ userId: 1 }).then(function (user) {
...
describe('League DB Queries', function () {
  ...
  it('Should insert a new league into the DB', function (done) {
    ...
    leagueDB.addLeague({ name: 'targaryan', user_id: 1 }).then(function (leagueCreated) {

      leagueDB.findLeague({ leagueId: 1 }).then(function (league) {
  ...
  it('Should update league with a different user_id', function (done) {
    ...
    authDB.addNewUser(users[1]).then(function (results) {
      // then update league with new user_id

      leagueDB.updateLeague({ userId: 1, newUserId: 2 }).then(function (idUpdated) {
        // Should also update users table: old user isMod to false
        // And update newUser isMod to true
        userDB.updateUser({ isModerator: false }, 1).then(function (updated) {
          userDB.updateUser({ isModerator: true }, 2).then(function (updated) {
            // verify league was updated
            leagueDB.findLeague({ leagueId: 1 }).then(function (leagueFound) {
  ...
  it('Should update league name', function (done) {
    leagueDB.updateLeague({ name: 'targaryan', newName: 'johnsnow' }).then(function (results) {
      leagueDB.findLeague({ leagueId: 1 }).then(function (league) {
...
describe('User DB Queries', function () {
  ...
  it('Should update users league', function (done) {
    userDB.updateUser({ league_id: 1 }, 2).then(function (updated) {
      authDB.findUser({ userId: 2 }).then(function (user) {
...
describe('Characters DB Queries', function (done) {
  it('Should insert a new character into the DB', function (done) {
    characterDB.addCharacter(characters[0]).then(function (character) {
...
describe('Events DB Queries', function () {
  it('Should insert a new event into the DB', function (done) {
    eventDB.addEvent(events[0]).then(function (event) {
...
describe('Helper DB Queries', function () {
  it('Should insert user roster data into ROSTER_DATA', function (done) {
    helperDB.addCharacter({ league_id: 1, user_id: 2, char_id: 1, episode: 1 }).then(function (character) {
      expect(character.insertId).to.equal(1);
      done();
    }).catch(function (err) {
